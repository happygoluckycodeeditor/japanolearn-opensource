...
const LessonPage: React.FC = () => {
  ...
  // Check for lesson completion and trigger confetti ONCE
  useEffect(() => {
    const totalProgress = Math.round(videoProgress + quizProgress)

    // Only trigger confetti once when reaching 100%
    if (totalProgress === 100 && !hasTriggeredConfetti) {
      setHasTriggeredConfetti(true)
      setShowConfetti(true)

      // Stop confetti after exactly 3 seconds
      confettiTimeoutRef.current = setTimeout(() => {
        setShowConfetti(false)
      }, 7000)
    }
  }, [videoProgress, quizProgress, hasTriggeredConfetti])
  ...
  return (
    <>
      {/* Confetti Component - Fixed to viewport */}
      {showConfetti && (
        <div className="fixed inset-0 pointer-events-none z-[9999]">
          <Confetti
            ...
            gravity={0.25}
            initialVelocityY={10}
            colors={['#f43f5e', '#10b981', '#3b82f6', '#f59e0b', '#8b5cf6', '#ef4444', '#06b6d4']}
            confettiSource={{
              x: 0,
              y: 0,
              w: window.innerWidth,
