...
const AllLessons: React.FC = () => {
  ...
  const beginnerCount = lessons.filter(
    (lesson) => lesson.level?.toLowerCase() === 'beginner'
  ).length
  const intermediateCount = lessons.filter(
    (lesson) => lesson.level?.toLowerCase() === 'intermediate'
  ).length
  const advancedCount = lessons.filter(
    (lesson) => lesson.level?.toLowerCase() === 'advanced'
  ).length
  ...
  return (
    <div className="w-screen max-w-full p-10 pt-20 sm:pl-10 sm:pr-10 md:pl-24 md:pr-24 bg-gray-200 min-h-screen">
      ...
      <p className="mb-8 text-lg">
        Choose your skill level to start learning Japanese, or browse lessons by category.
      </p>
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-3 mb-12">
        ...
        >
          <div className="card-body text-center">
            ...
            <p>
              Master complex grammar patterns, nuanced expressions, and advanced reading skills.
            </p>
            <div className="card-actions justify-center mt-4">
      ...
      <div className="mb-8">
        ...
        <div className="flex flex-wrap gap-3">
          {categories.map((category) => (
            <Link key={category} to={`/category/${category}`} className="btn btn-outline">
              {category}
            </Link>
          ))}
        </div>
